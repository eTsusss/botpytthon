...
def save_user_to_excel(username, email, user_id):
    if not os.path.exists(EXCEL_FILE):
        wb = Workbook(); ws = wb.active; ws.append(["Username", "Email", "UserID"]); wb.save(EXCEL_FILE)
    wb = load_workbook(EXCEL_FILE); ws = wb.active; ws.append([username, email, user_id]); wb.save(EXCEL_FILE)

def is_user_registered(username):
    if not os.path.exists(EXCEL_FILE): return False
    wb = load_workbook(EXCEL_FILE); ws = wb.active
    for row in ws.iter_rows(min_row=2, values_only=True):
        if row[0] == username: return True
    return False

def get_all_user_ids():
    if not os.path.exists(EXCEL_FILE): return []
    wb = load_workbook(EXCEL_FILE); ws = wb.active; user_ids = set()
    for row in ws.iter_rows(min_row=2, values_only=True):
        if len(row) > 2 and row[2]: user_ids.add(row[2])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_user_registered(username):
        ...
        return
    await update.message.reply_text(
        WELCOME_TEXT.format(username=update.effective_user.first_name or "друг"),
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Зарегистрироваться", callback_data="register")]])
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "register":
        ...
        try:
            with open(VIDEO_PATH, "rb") as v: await query.message.reply_video(v)
            with open(PDF_PATH, "rb") as p: await query.message.reply_document(p)
        except Exception as e:
            await query.message.reply_text("Ошибка при отправке файлов.")
...
async def broadcast_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    link = None if update.message.text.lower() == 'нет' else update.message.text
    context.user_data['broadcast_link'] = link
    user_ids = get_all_user_ids()
    count = 0
    for uid in user_ids:
        try:
            reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Перейти", url=link)]]) if link else None
            if context.user_data['broadcast_photo']:
                await context.bot.send_photo(uid, context.user_data['broadcast_photo'], caption=context.user_data['broadcast_text'], reply_markup=reply_markup)
            else:
                await context.bot.send_message(uid, text=context.user_data['broadcast_text'], reply_markup=reply_markup)
            count += 1
        except Exception:
            continue
...
async def get_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id in ADMIN_USER_IDS:
        if os.path.exists(EXCEL_FILE): await update.message.reply_document(open(EXCEL_FILE, "rb"))
